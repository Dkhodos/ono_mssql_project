...
describe("SQL Join data", () => {
    ...
    test("Interaction Dates Selection Based on Source and Tag Count", async () => {
        ...
        const interactions = [
            new Interaction({user_id: userIDs[0], content_id: contentIDs[0], type: 'Like', date: new Date('2024-01-01')}),
            new Interaction({user_id: userIDs[1], content_id: contentIDs[1], type: 'Like', date: new Date('2024-01-02')}),
            new Interaction({user_id: userIDs[2], content_id: contentIDs[2], type: 'Comment', date: new Date('2024-01-03')})
        ];
        await sqlServer.execute(interactionGenerator.generateQuery(interactions));

        const action = new SqlAction(sqlServer, 'select_interaction_dates_based_on_tags.sql');
